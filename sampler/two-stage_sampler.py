...
class SpreadoutSampler(Sampler):
    ...
    def _negative_sampling(self, user_item_pairs):
        neg_samples = np.zeros(shape=(user_item_pairs.shape[0], self.n_negatives))
        pos_item_embeddings = self.model.dense_item_embeddings[pos_ids] # (batch, dim)
        neg_item_embeddings = self.model.dense_item_embeddings[neg_ids] # (C, dim)

        item_spreadout_distances = np.linalg.multi_dot([pos_item_embeddings,
                                                        neg_item_embeddings.T]) # (batch, C)
